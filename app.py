import os
import traceback
import streamlit as st
import speech_recognition as sr
from transformers import pipeline

st.set_page_config(layout="wide")
st.title("🎧 SPEECH EMOTION RECOGNITION  📝")

st.sidebar.title("BATCH-0X")
st.sidebar.write("SPEECH EMOTION RECOGNITION")
st.sidebar.write("===========================")

# Aligning "Upload Audio" section to the center of the sidebar
st.sidebar.markdown(
    """
    <style>
        .sidebar .block-container {text-align: center;}
    </style>
    """,
    unsafe_allow_html=True,
)

st.sidebar.header("Upload Audio")
audio_file = st.sidebar.file_uploader("", type=["wav"])
st.sidebar.write(" ")
upload_button = st.sidebar.button("Upload")
st.sidebar.write(" ")

voice_input_button = st.sidebar.button("Start Voice Input")

def perform_sentiment_analysis(text):
    model_name = "distilbert-base-uncased-finetuned-sst-2-english"
    sentiment_analysis = pipeline("sentiment-analysis", model=model_name)
    results = sentiment_analysis(text)
    sentiment_label = results[0]['label']
    sentiment_score = results[0]['score']
    return sentiment_label, sentiment_score

def transcribe_audio(audio_file):
    r = sr.Recognizer()
    with sr.AudioFile(audio_file) as source:
        audio = r.record(source)
        transcribed_text = r.recognize_google(audio)
    return transcribed_text

def main():
    if voice_input_button:
        # Use microphone for voice input
        r = sr.Recognizer()
        with sr.Microphone() as source:
            st.sidebar.write("Listening...")
            audio = r.listen(source)

        try:
            st.sidebar.write("Processing...")
            voice_input_text = r.recognize_google(audio)
            st.sidebar.write("Voice Input:", voice_input_text)

            # Perform sentiment analysis on the voice input text
            sentiment_label, sentiment_score = perform_sentiment_analysis(voice_input_text)

            st.header("Sentiment Analysis")
            negative_icon = "👎"
            neutral_icon = "😐"
            positive_icon = "👍"
            
            if sentiment_label == "NEGATIVE":
                st.write(f"{negative_icon} Negative (Score: {sentiment_score})", unsafe_allow_html=True)
            elif sentiment_label == "NEUTRAL":
                st.write(f"{neutral_icon} Neutral (Score: {sentiment_score})", unsafe_allow_html=True)
            elif sentiment_label == "POSITIVE":
                st.write(f"{positive_icon} Positive (Score: {sentiment_score})", unsafe_allow_html=True)

            st.info(
                "The sentiment score measures how strongly positive, negative, or neutral the feelings or opinions are."
                "A higher score indicates a positive sentiment, while a lower score indicates a negative sentiment."
            )
        except Exception as ex:
            st.error("Error occurred during voice input and sentiment analysis.")
            st.error(str(ex))
            traceback.print_exc()

    elif audio_file and upload_button:
        # Process uploaded audio file
        try:
            transcribed_text = transcribe_audio(audio_file)
            sentiment_label, sentiment_score = perform_sentiment_analysis(transcribed_text)

            st.header("Transcribed Text")
            st.text_area("Transcribed Text", transcribed_text, height=200)
            
            st.header("Sentiment Analysis")
            negative_icon = "👎"
            neutral_icon = "😐"
            positive_icon = "👍"
            
            if sentiment_label == "NEGATIVE":
                st.write(f"{negative_icon} Negative (Score: {sentiment_score})", unsafe_allow_html=True)
            elif sentiment_label == "NEUTRAL":
                st.write(f"{neutral_icon} Neutral (Score: {sentiment_score})", unsafe_allow_html=True)
            elif sentiment_label == "POSITIVE":
                st.write(f"{positive_icon} Positive (Score: {sentiment_score})", unsafe_allow_html=True)

            st.info(
                "The sentiment score measures how strongly positive, negative, or neutral the feelings or opinions are."
                "A higher score indicates a positive sentiment, while a lower score indicates a negative sentiment."
            )
        except Exception as ex:
            st.error("Error occurred during audio transcription and sentiment analysis.")
            st.error(str(ex))
            traceback.print_exc()

if __name__ == "__main__":
    main()
